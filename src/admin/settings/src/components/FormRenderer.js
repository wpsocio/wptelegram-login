...
import { BotTestResult, Card, FormField, Instructions } from './';
import { getFieldLabel } from '../fields';
import { testBotToken } from '../utils/TelegramUtils';

const { select_opts } = settings;
...
const FormRenderer = ({
	...
	values,
	errors,
	form: { mutators: _mutators, getState },
	setFormState,
}) => {
		...
		<Form onSubmit={handleSubmit}>
							...
							<Button
								...
								onClick={() =>
									testBotToken({
										...
										setTestingBotToken,
										setBotTokenTestResult,
										setBotTokenTestResultType,
										mutators: _mutators,
									})
								}
							>
			...
			<Card title={__('Login Options')}>
				<FormField
					...
					desc={__(
						'If checked, only the existing users who have connected their Telegram will be able to login.'
					)}
					controlProps={{
						id: 'disable_signup',
						type: 'checkbox',
				...
				{!values.disable_signup && (
					<FormField
						...
						_label={__('Generate a random email address')}
						desc={
							__('If checked, a random email address will be generated for new user accounts.') +
							' ' +
