(function( $, I18n ) {
    ...
    app1.init = function () {
        ...
        app1.metabox.on( 'change', 'input[type=checkbox][name=disable_signup],input[type=checkbox][name=show_message_on_error]', app1.toggle_dependents );
        ...
        // trigger on ready
        app1.metabox.find('input[type=checkbox][name=disable_signup],input[type=checkbox][name=show_message_on_error]').trigger('change');
    };
    app1.test_bot_token = function(){
    	if (!app1.bot_token.val().trim()) {
    		window.alert(I18n.empty_bot_token);
    		return;
    	}
    ...
    app1.check_redirect_to = function () {
    ...
    app1.toggle_dependents = function () {
        var $this = $(this);
        var id = $this.attr('id');
        ...
        var deps = app1.metabox.find('[data-the-conditional-id="'+id+'"]');

        deps.each(function() {
            var row = $(this).closest('.cmb-row');
            if ( val === $(this).attr('data-the-conditional-value') ) {
                row.show();
            } else {
                row.hide();
            }
	...
	app1.fix_corner_radius = function ( event ) {
		...
		var form = app1.metabox.closest('#wptelegram_login').get(0), corner_radius;
		if ('corner_radius' == event.target.id) {
			corner_radius = form.corner_radius.value.replace(/[^0-9]+/g, '');
			form.corner_radius.realValue = corner_radius;
		} else {
			corner_radius = (form.corner_radius.realValue || form.corner_radius.defaultValue).replace(/[^0-9]+/g, '');
			form.corner_radius.value = corner_radius;
		}
		if (''==corner_radius) {
			return;
		}
		corner_radius = parseInt(corner_radius, 10);
		if (corner_radius <= 0) {
    ...
    app1.handle_bot_token_test = function( jqXHR ) {
